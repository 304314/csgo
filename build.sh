...
export OPENEULER_ACPO_DIR="" # ACPO dir, need download.
export TENSORFLOW_AOT_PATH="" # Tensorflow dir. eg: PYTHON_DIR/site-packages/tensorflow
...
if [ $acpo_aot -eq 1 ] && [ $override_aot -eq 0 ]; then
  if [ -z "$TENSORFLOW_AOT_PATH" ]; then
    echo "$0: TENSORFLOW_AOT_PATH not set, eg: export TENSORFLOW_AOT_PATH=PYTHON_DIR/site-packages/tensorflow"
    exit 1
  else
    echo "$0: tensorflow is found at $TENSORFLOW_AOT_PATH"
  fi
fi
